...
// Notes
// The size parameter represents the maximum size for each sub-array (see ex.4). You should try to fill each sub-array evenly. In other words, 
ex.4 should be [[1, 3, 5], [2, 4, 6]], not [[1, 3, 5, 6], [2, 4]].
...
function group(arr, size) {
	let next=Math.round(arr.length/size)
  
  let result=[]
  let group
  for(let i=0;i<next;i++){
     group=[]
    for(let j=i;j<arr.length;j=j+next){
      group=[...group,arr[j]]
    }
    result=[...result,group]
  }
