/* If a number is autobiographical, then the number's digits describe itself. The first digit is the amount of times the digit 0 appears, 
the second digit is the amount of times the digit 1 appears, the third digit is the amount of times the digit 2 appears, etc. This repeats 
for all digits of the number. The number 6210001000 is autobiographical, because it has six 0's, two 1's, one 2, zero 3's, zero 4's, zero 5's,
 one 6, zero 7's, zero 8's, and zero 9's.
...
function isAutobiographical(n) {
    ...
    let v = number.every((val, index) => {
        let counter = 0
        number.forEach(val1 => {
            if (index == parseInt(val1)) {
                counter++
            }
        })
...
console.log(isAutobiographical(0))