...
The two players place randomly the 26 letters of the English alphabet in a string and ten integers (ranged from 0 up to 25) in an array. Each integer
 represents the index of a letter to eliminate from the opponent's string so that the two players will afford the battle with two reduced strings of 16 letters.
...
Now, you have to compare the letters of the players' strings progressively from the left, obtaining their ASCII values: during this series of clashes, the
 highest letter (meaning the greater ASCII value) gives to his possessor a number of points equal to its value less the one of its opponent. If the letters 
are equal for both players, they don't gain any points.
...
Points are gained by the possessor of the highest letter in every single clash, equal to the ASCII value of the higher letter less than the ASCII value of 
the lower letter. If two letters are the same it's a draw, and nothing happens.
...
function alphaClash(str_A, ind_A, str_Z, ind_Z) {
  ...
  for (let i = 0; i < p1.length; i++) {
    if (p1.charCodeAt(i) > p2.charCodeAt(i)) {
      p1score = p1score + (p1.charCodeAt(i) - p2.charCodeAt(i))
    } else if (p1.charCodeAt(i) < p2.charCodeAt(i)) {
      p2score = p2score + (p2.charCodeAt(i) - p1.charCodeAt(i))
    }
  }
  return { A: p1score, Z: p2score }
}
