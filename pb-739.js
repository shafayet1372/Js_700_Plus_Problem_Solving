...
Your open-plan office building has a scrolling message screen on the far wall. One day, you notice that the messages are starting to glitch.
 Some of the lower case letters are being replaced by their position in the alphabet ("a" = 1, "b" = 2, ..., "z" = 26). Given the glitched text, return the corrected message.
...
function messageGlitch(txt) {
  ...
  for (let i = 0; i < s.length; i++) {
    if ((!isNaN(s[i]) && !isNaN(s[i + 1])) && (s[i] != ' ' && s[i + 1] != ' ')) {
      result.push(String.fromCharCode(96 + parseInt(s.slice(i, i + 2).join(""))))
      i++
    } else if (!isNaN(s[i]) && s[i] != ' ') {
      result.push(String.fromCharCode(96 + parseInt(s.slice(i, i + 1).join(""))))
    } else {
      result.push(s[i])
    }
