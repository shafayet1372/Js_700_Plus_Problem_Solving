/* Parseltongue
Hermione has come up with a precise formula for determining whether or not a phrase was ssspoken by a parssseltongue (a reference from the 
Harry Potter universe; the language of ssserpents and those who can converse with them).
...
function isParselTongue(sentence) {
  ...
  return s.every(x => {
    return /(ss)+/i.test(x) || /^[^s]+$/i.test(x)
  })
}

console.log(isParselTongue("she ssselects to eat that apple. "))