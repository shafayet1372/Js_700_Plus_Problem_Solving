/* In this challenge, you have to establish if an integer is an Unprimeable number. To be Unprimeable, when a single digit of a composite number is 
exchanged with any digit from 0 up to 9, the new number obtained must not be a prime:
...
Despite is still an unproofed theory, as far as we know every even number (except 2) is not prime. You are free to choose to use this discriminant when you 
check the number obtained after the change of a digit.
The same concept can be applied also to primes. Primes that do not return other primes when their single digits are changed (apart from the number itself) 
are called Weakly Primes: the first prime of this series is 294001. */
    ...
    for (let i = 0; i < number.length; i++) {
      ...
      for (let j = 0; j <= 9; j++) {
        if (i == number.length - 1) {
          value = number.slice(0, i).join("") + j
        } else if (i == 0) {
          value = j + number.slice(i + 1, number.length).join("")
        } else {
          value = number.slice(0, i).join("") + j + number.slice(i + 1, number.length).join("")
        }
        if (primeCheck(parseInt(value))) {
          console.log(value)
