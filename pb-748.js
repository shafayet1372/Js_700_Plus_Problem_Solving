...
A magic sigil is a glyph which represents a desire one wishes to manifest in their lives. There are many ways to create a sigil, but the most common is 
to write out a specific desire (e.g. "I HAVE WONDERFUL FRIENDS WHO LOVE ME"), remove all vowels, remove any duplicate letters (keeping the last occurence), and then design a glyph from what remains.
...
function sigilize(desire) {
  let reg = /[aeiou]/ig
  let s = desire.split(reg).join("").split("")
  for (let i = 0; i < s.length; i++) {

    if (s.slice(i + 1, s.length).join("").includes(s[i])) {

      s[i] = ''
    }
  }

  return s.join("").replace(/\s+/, '').toUpperCase()
}
console.log(sigilize("My business is financially successful"))