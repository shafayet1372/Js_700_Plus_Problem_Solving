...
In this question you will be given a table as below, where the first row lists the names of products, and each of row after that lists the sales 
...
function totalSales(sales, product) {
  ...
  for (let i = 0; i < sales.length; i++) {
    let proList = sales[i]
    if (proList.indexOf(product) == -1) {
      return 'Product not found'
    } else {
      sum = 0
      let index = proList.indexOf(product)
