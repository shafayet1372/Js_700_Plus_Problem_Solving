/* Briscola! (Part I)
Briscola is an Italian card game, played with a deck of 40 cards that has four suits (hearts, diamonds, clubs, and spades), so that there are
 ten cards per suit: the Ace, the numbered cards from 2 up to 7, and the three face-cards (Jack, Queen, and King). In this challenge, the notation 
used for the cards is a string containing the card value (with the upper-case initial for face-cards) and the lower-case initial for suits, as in the examples below:
...
Each game of Briscola is made of two rounds. After the first round, the points are counted for both you and your opponent, and these scores (plus 1) 
will set the target for winning the game, after that the second round is played.
...
The original standard suits and face-cards of an Italian deck are different from the international ones used for Poker. If you want to know more, take 
a look at the Resources tab. */
let cards = {
  "A": 11, "T": 10, "K": 4, "Q": 3, "J": 2, '3': 10
}
...
function briscolaScore(myDeck1, myDeck2) {
  ...
  let opfRound = 120 - myfRound
  let mysRound = myDeck2.map(val => {
    let s = val.split("")
    if (cards[s[0]]) {
      return cards[s[0]]
    }
  ...
  if (mysRound > opfRound) {
    return "You Win!"
  } else if (mysRound < opfRound) {
    return 'You Lose!'
  }
  return 'Draw!'
}

console.log(briscolaScore(["3c", "3s", "Qd", "Jh", "5d", "Jc", "6d", "Ad", "Js", "Qc"],
  ["Jd", "Kd", "4c", "6s", "Ks", "5c", "3d", "As", "Jh", "6h"]))